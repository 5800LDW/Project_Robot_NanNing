...
public class DanceMusicListActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    private List<DanceBean> danceList;
...
//        desktopManager.doReset();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView)findViewById(com.example.xukefeng.musicplayer.R.id.appItemPageBackTitleTV))
                .setText(getString(com.example.xukefeng.musicplayer.R.string.base_text_dance));

        findViewById(com.example.xukefeng.musicplayer.R.id.appItemPageBack).setOnClickListener(v ->{
            myFinish();
        });
        ...
        speechManager.setOnSpeechListener(new BaseRecognizeListener() {
            ...
            public void voiceRecognizeText(String voiceTXT) {
                if(!ActivityUtils.isForeground(DanceMusicListActivity.this)){
                    return;
                }
                //语音监听返回
...
//        stopService(intent) ;
        ...
        DanceManager manager = MainApp.getInstance().getManager();
        if (manager != null) {
            manager.destroy();
        }
    }
    ...
    private void init() {
        ...
        danceList = danceManager.getDanceList(this);
        title.clear();
        List_map = new ArrayList<Map<String, String>>();
        if (danceList == null || danceList.size() == 0) {
        ...
        for (int i = 0; i < danceList.size(); i++) {
            ...
            map.put("index", (i + 1) + " " + danceList.get(i).getName());
            List_map.add(map);
        }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        String str = danceList.get(position).getSongUri();
        if(!new File(str).exists()){
            speakAndCheckComplete("舞蹈没有关联媒体文件，请使用舞蹈程序进行关联", () -> {
                //将点击位置传递给播放界面，在播放界面获取相应的音乐信息再播放。
                //将点击位置传递给播放界面，在播放界面获取相应的音乐信息再播放。
            });
            return;
